...
export class SeedInitialSchema1759958275364 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`slc_tools_catalog\` (\`platform_name\` varchar(255) NOT NULL, \`url\` varchar(255) NOT NULL, \`tool_description\` varchar(255) NULL, \`license\` varchar(255) NULL, \`standard_support\` varchar(255) NULL, \`keywords\` varchar(255) NULL, \`contact_email\` varchar(255) NULL, PRIMARY KEY (\`platform_name\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`dataset_catalog\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`platform\` varchar(255) NOT NULL, \`datasetName\` varchar(255) NOT NULL, \`url\` varchar(255) NOT NULL, \`description\` text NOT NULL, \`dataFormats\` json NOT NULL, \`dataType\` json NOT NULL, \`keywords\` json NOT NULL, \`population\` varchar(255) NULL, \`contributors\` json NULL, \`language\` varchar(255) NULL, \`publicationYear\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`item_classification\` (\`id\` int NOT NULL AUTO_INCREMENT, \`classified_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`item_id\` int NOT NULL, \`class_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`validation_results\` ADD CONSTRAINT \`FK_0158ca2c3ffd1a19d9d4d783325\` FOREIGN KEY (\`itemId\`) REFERENCES \`slc_item_catalog\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
