...
export class SeedInitialData1764779460569 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`item_classification\` (\`id\` int NOT NULL AUTO_INCREMENT, \`classified_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`item_id\` varchar(255) NOT NULL, \`class_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`validation_results\` ADD CONSTRAINT \`FK_582de40a329540a2f412f5fe017\` FOREIGN KEY (\`itemPersistentID\`) REFERENCES \`slc_item_catalog\`(\`persistentID\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`item_classification\` ADD CONSTRAINT \`FK_23a557b6c75301ed489de065d6f\` FOREIGN KEY (\`item_id\`) REFERENCES \`slc_item_catalog\`(\`persistentID\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
